...
echo "   4) Install Docker without upgrade"
...
case "$option" in
    ...
    ;;
    4)
      read -p "Are you sure to Install Docker without upgrade? [y/n]: " dockerw
      until [[ "$dockerw" =~ ^[yYnN]*$ ]]; do
          echo "$dockerw: invalid selection."
          read -p "Are you sure to Install Docker without upgrade? [y/n]: " dockerw
      done
      if [[ "$dockerw" =~ ^[yY]$ ]]; then
          sudo apt-get update -y;
          ...
          echo
          read -p "Do you want to install docker with upgrade? [y/n]:" dockeru
          until [[ "$dockeru" =~ ^[yYnN]*$ ]]; do
              echo "$dockeru: invalid selection."
              read -p "Do you want to install docker with upgrade? [y/n]: " dockeru
          done
          if [[ "$dockeru" =~ ^[yY]$ ]]; then
              sudo apt-get update -y;
